...
console.log(upgraded(1, 2, 3)); // вычисляем: 9  
console.log(upgraded()); //вычисляем NaN
...
setTimeout(() => upgradedSendSignal(1, 0), 0); // Сигнал отправлен + будет запланирован асинхронный запуск, который будет проигнорирован, так как следующий сигнал отменит предыдущий (300 - 0 < 2000)
...
setTimeout(() => {
  console.log("count", upgradedSendSignal.count()); // было выполнено 3 отправки сигнала
  console.log("allCount", upgradedSendSignal.allCount()); // было выполнено 7 вызовов декорированной функции
}, 7000)
